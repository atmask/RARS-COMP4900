/*
 * dehumidifer_actuator.c
 *
 *	Driver for actuating the dehumidifier.
 *	Receives messages from the the humidity_manager and sends commands to the environment simulation process
 *
...
void *runPublisher(void *args){
	...
	log_file = fopen("/tmp/dehumidifier_actuator_publisher.log", "w");
	logString(log_file, "Starting dehumidifier actuator publisher");
	...
	while(running)
	{
		...
		if(state_changed_flag)
		{
			...
			if(MsgSendPulse(envSim_coid, -1, DEHUMIDIFIER_ACTUATOR_CHANGE, state) == 1){
...
void *runServer(void *args){
	...
	//set up log file
	log_file = fopen("/tmp/dehumidifier_actuator_server.log", "w");
	logString(log_file, "Starting dehumidifier actuator server");

	// register our name for a channel
	attach = name_attach(NULL, DEHUMIDIFIER_ACTUATOR_SERVER, 0);
	if (attach == NULL){
