...
/*
 *  our mutex variable for the humid data
 */

pthread_mutex_t humid_data_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int main(void) {
	...
	FILE *log_file1;
	log_file1 = fopen("/tmp/humid_sensor_reader.log", "w");
	logString(log_file1, "Starting humid sensor");

	FILE *log_file2;
	log_file2 = fopen("/tmp/humid_sensor_server.log", "w");
	logString(log_file2, "Starting humid sensor");
...
void *runServer(void *args){
	...
	attach = name_attach(NULL, HUMIDITY_SENSOR_SERVER, 0);
	if (attach == NULL){
	...
	while (running) {
		...
		if (0 == rcvid) {
			...
			switch(rbuf.type){
			case GET_DATA:
				// Get the mutex and return the humid data
				pthread_mutex_lock(&humid_data_mutex);
				logString(log_file, "Return data: %f", humid_data);
				/* Build the response */
				resp_snsr_data_msg_t resp;
				resp.data = humid_data;
