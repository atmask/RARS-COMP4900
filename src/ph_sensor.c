...
/*
 *  our mutex variable for the ph data
 */

pthread_mutex_t ph_data_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int main(void) {
	...
	FILE *log_file1;
	log_file1 = fopen("/tmp/ph_sensor_reader.log", "w");
	logString(log_file1, "Starting ph sensor");

	FILE *log_file2;
	log_file2 = fopen("/tmp/ph_sensor_server.log", "w");
	logString(log_file2, "Starting ph sensor");
...
void *readData(void *args){
	...
	while(running){
		/* Lock the mutex */
		pthread_mutex_lock(&ph_data_mutex);
		/* Scanf to get the ph
		 * N.B. Scanf reads up the whitespace
		 * */
		fscanf(stdin, "%f", &ph_data);
		logString(log_file, "Read data: %f", ph_data);

		pthread_mutex_unlock(&ph_data_mutex);
...
void *runServer(void *args){
	...
	attach = name_attach(NULL, PH_SENSOR_SERVER, 0);
	if (attach == NULL){
	...
	while (running) {
		...
		if (0 == rcvid) {
			...
			switch(rbuf.type){
			case GET_DATA:
				// Get the mutex and return the ph data
				pthread_mutex_lock(&ph_data_mutex);
				logString(log_file, "Return data: %f", ph_data);
				/* Build the response */
