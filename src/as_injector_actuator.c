/*
 * as_injector_actuator.c
 *
 *	Driver for actuating the aluminum sulfate injector actuator. Aluminum sulfate is used to lower the pH level of soil.
 *	Receives messages from the the pH_manager and sends commands to the environment simulation process
 *
 *  Created on: Dec. 4th, 2022
 */
#include <stdio.h>
#include <unistd.h>
...
void *runPublisher(void *args){
	...
	log_file = fopen("/tmp/as_injector_actuator_publisher.log", "w");
	logString(log_file, "Starting aluminum sulfate injector actuator publisher");
	...
	while(running)
	{
		...
		if(state_changed_flag)
		{
			...
			if(MsgSendPulse(envSim_coid, -1, AS_INJECTOR_ACTUATOR_CHANGE, state) == 1){
...
void *runServer(void *args){
	...
	//set up log file
	log_file = fopen("/tmp/as_injector_actuator_server.log", "w");
	logString(log_file, "Starting aluminum sulfate injector actuator server");

	// register our name for a channel
	attach = name_attach(NULL, AS_INJECTOR_ACTUATOR_SERVER, 0);
	if (attach == NULL){
