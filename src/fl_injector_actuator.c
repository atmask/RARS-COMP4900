/*
 * fl_injector_actuator.c
 *
 *	Driver for actuating the flowable lime actuator. Flowable lime is used to raise the pH level of soil.
...
void *runPublisher(void *args){
	...
	log_file = fopen("/tmp/fl_injector_actuator_publisher.log", "w");
	logString(log_file, "Starting flowable lime injector actuator publisher");
	...
	while(running)
	{
		...
		if(state_changed_flag)
		{
			...
			if(MsgSendPulse(envSim_coid, -1, FL_INJECTOR_ACTUATOR_CHANGE, state) == 1){
...
void *runServer(void *args){
	...
	//set up log file
	log_file = fopen("/tmp/fl_injector_actuator_server.log", "w");
	logString(log_file, "Starting flowable lime injector actuator server");

	// register our name for a channel
	attach = name_attach(NULL, FL_INJECTOR_ACTUATOR_SERVER, 0);
	if (attach == NULL){
