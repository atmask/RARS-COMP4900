/*
 * humidifer_actuator.c
 *
 *	Driver for actuating the humidifier.
...
void *runPublisher(void *args){
	...
	log_file = fopen("/tmp/humidifier_actuator_publisher.log", "w");
	logString(log_file, "Starting humidifier actuator publisher");
	...
	while(running)
	{
		...
		if(state_changed_flag)
		{
			...
			if(MsgSendPulse(envSim_coid, -1, HUMIDIFIER_ACTUATOR_CHANGE, state) == 1){
...
void *runServer(void *args){
	...
	//set up log file
	log_file = fopen("/tmp/humidifier_actuator_server.log", "w");
	logString(log_file, "Starting humidifier actuator server");

	// register our name for a channel
	attach = name_attach(NULL, HUMIDIFIER_ACTUATOR_SERVER, 0);
	if (attach == NULL){
